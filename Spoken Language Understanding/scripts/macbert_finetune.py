...
#hfl/chinese-macbert-base
model = BertForSlu.from_pretrained('hfl/chinese-macbert-base',config=config, args=args)

if args.testing:
    check_point = torch.load(open('mac_model.bin','rb'),map_location=device)
...
def predict():
    ...
    with torch.no_grad():
        for i in range(0, len(test_dataset), args.batch_size):
            ...
            current_batch = from_example_list_Bert(args, cur_dataset, device, train=False)
