...
from utils.batch import from_example_list_Bert
...
model = BertForSlu.from_pretrained('hfl/chinese-roberta-wwm-ext',config=config, args=args)
check_point = torch.load(open('roberta_model.bin','rb'),map_location=device)
...
if not args.testing:
    ...
    optimizer = AdamW(model.parameters(), lr=args.lr) #Roberta
